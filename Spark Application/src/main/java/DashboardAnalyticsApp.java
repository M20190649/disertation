...
/*  private static final double refLat = 44.3369102;
  ...
  static void ClusterTaxiActions(JavaPairReceiverInputDStream<String, String> messages, final TaxiAction taxiActionArg, final JavaSparkContext sc) {
    ...
    }).foreachRDD(new Function<JavaRDD<org.apache.spark.mllib.linalg.Vector>, Void>() {
      @Override
      public Void call(JavaRDD<org.apache.spark.mllib.linalg.Vector> rdd) throws Exception {
        ...
        if(!rdd.partitions().isEmpty()) {
          ...
          for (org.apache.spark.mllib.linalg.Vector clusterCenter : clusters.clusterCenters()) {
            // Average per tile
            Row row = RowFactory.create(clusterCenter.toArray()[0], clusterCenter.toArray()[1], taxiActionArg.getValue());
            rows.add(row);
          }
